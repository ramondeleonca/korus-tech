...
export default function Index() {
	...
	useEffect(() => {
		const ctx = gsap.context(() => {
			...
			for (let child of polaroidsRef.current?.children ?? [])
				gsap.to(child, {
					rotate: `${15 + Math.floor(Math.random() * 15)}deg`,
					scrollTrigger: {
						scrub: true,
						trigger: polaroidsRef.current,
						start: "top 25%",
	...
	return (
		<>
			...
			<ReactLenis ref={lenisRef} root autoRaf={false} options={{lerp: 0.05, smoothTouch: false} as ReactLenisOptions}>
				{/* Home section */}
				<section className="hero relative overflow-hidden" id="home">
					...
					<div className="background-container h-full max-w-5xl left-0 w-[100vw] absolute -z-[1] background themed" data-scroll data-scroll-speed="-2.5">
						<Image fetchPriority="high" loading="eager" priority alt="A dynamic portrait of a person looking for lyrical inspiration and song information" style={{ objectFit: "cover" }} fill={true} src={homeBackground?.image ? `/backgrounds/random/${homeBackground?.image}` : ""}></Image>
					</div>
